...
for key, message, index in zip(keys, messages, range(0, 6)):
    ...
    assert authentication_tags['hmac_sha_224'][index] == hmac_sha_224.compute(
        key, message).hex()
    hmac_sha_256 = HMAC(hash_algorithms[1])
    assert authentication_tags['hmac_sha_256'][index] == hmac_sha_256.compute(
        key, message).hex()
    hmac_sha_384 = HMAC(hash_algorithms[2])
    assert authentication_tags['hmac_sha_384'][index] == hmac_sha_384.compute(
        key, message).hex()
    hmac_sha_512 = HMAC(hash_algorithms[3])
    assert authentication_tags['hmac_sha_512'][index] == hmac_sha_512.compute(
        key, message).hex()
